...
// Flags holds the command-line configuration for the SWAGGER scan module.
...
// implementation.
type Results struct {
	// Response body matched a Swagger file
	MatchedSwagger bool `json:"matched_swagger"`
...
//
// junn: resolves the hostname or ip and opens a tcp connection with timeout.
...
func (scan *scan) getCheckRedirect() func(*http.Request, *http.Response, []*http.Request) error {
	return func(req *http.Request, res *http.Response, via []*http.Request) error {
		...
		b := new(bytes.Buffer)
		bytesRead, _ := io.CopyN(b, res.Body, res.ContentLength)
...
func (scanner *Scanner) newSwaggerScan(t *zgrab2.ScanTarget, useHTTPS bool) *scan {
	ret := scan{
		scanner: scanner,
		target:  t,
		transport: &http.Transport{
...
func (scan *scan) IsSwaggerFile(haystack []byte) bool {
	isMatch := scan.scanner.fingerprint.Match(haystack)
	return isMatch
}

// Grab performs the HTTP scan -- implementation taken from zgrab/zlib/grabber.go
//
// junn: looks like the re
func (scan *scan) Grab() *zgrab2.ScanError {
	...
	if resp != nil && resp.Body != nil {
		...
		if scan.IsSwaggerFile(buf.Bytes()) {
			log.WithFields(log.Fields{
				"url": scan.url,
			}).Info("found a swagger file")
			...
			scan.results.MatchedSwagger = true
			...
			return zgrab2.NewScanError(zgrab2.SCAN_MATCH_ERROR, err)
		}
	}
...
func (scanner *Scanner) Scan(t zgrab2.ScanTarget) (zgrab2.ScanStatus, interface{}, error) {
	scan := scanner.newSwaggerScan(&t, scanner.config.UseHTTPS)
	defer scan.Cleanup()
	err := scan.Grab()

	if err != nil && err.Status != zgrab2.SCAN_MATCH_ERROR {
		if scanner.config.RetryHTTPS && !scanner.config.UseHTTPS {
			scan.Cleanup()
			retry := scanner.newSwaggerScan(&t, true)
			defer retry.Cleanup()
...
func RegisterModule() {
	...
	_, err := zgrab2.AddCommand("swagger", "Grab Swagger files", module.Description(), 80, &module)
	if err != nil {
		log.Fatal(err)
	}
}
